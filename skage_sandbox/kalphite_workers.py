...
def move_to_position(unit_pos, pos, prev_unit_pos=20):
    directions = np.arange(1, 5)
    direction = direction_to(unit_pos, pos)
    if np.array_equal(unit_pos, prev_unit_pos):
        direction = np.random.choice(np.delete(directions, direction-1, axis=None), size = 1)[0]
    return direction
...
class KQ():
    def __init__(self):
        ...
        #Starting from just above top left square of 5x5 grid
        self.scout_move_set = [3, 3, 3, 3, 3, 2, 1, 1, 1, 1, 2, 3, 2, 3, 3, 4, 3, 2, 2, 2, 2, 2, 2, 4]
    ...
    def act(self, step: int, obs, remainingOverageTime: int = 60):
        ...
        for idx, unit_id in enumerate(available_unit_ids):
            ...
            if len(self.relic_node_positions) > 0:
                ...
                relic_grid = create_relic_cluster(self.relic_node_positions[0])
                ##King logic
                if unit_id == 0:
                    manhattan_distance = abs(unit_pos[0] - nearest_relic_node_position[0]) + abs(unit_pos[1] - nearest_relic_node_position[1])
                    if manhattan_distance > 0:
                        actions[unit_id] = [move_to_position(unit_pos, nearest_relic_node_position, self.prev_positions[unit_id]), 0, 0]
                    
                    else:
                        self.KING_in_position = True
                ...
                ##Queen logic
                    ...
                    start_of_route = [nearest_relic_node_position[0] - 2, nearest_relic_node_position[1] - 3]
                    if self.KQ.searching == False:
                        self.KQ.start_point = start_of_route
                        manhattan_distance = abs(unit_pos[0] - start_of_route[0]) + abs(unit_pos[1] - start_of_route[1])
                        if manhattan_distance > 0:
                            actions[unit_id] = [move_to_position(unit_pos, start_of_route, self.prev_positions[unit_id]), 0, 0]
                        else:
                            self.KQ.searching = True
                            actions[unit_id] = [self.KQ.scout_move_set.pop(0), 0, 0]
                    else:
                        expected_point_gain = 1 if self.KING_in_position else 0

                        gained_points = obs['team_points'][self.team_id] - self.prev_points
                        if gained_points > expected_point_gain:
                            self.relic_fragments.append(unit_pos)
                        
                        if len(self.KQ.scout_move_set) > 0:
                            actions[unit_id] = [self.KQ.scout_move_set.pop(0), 0, 0]
                        else:
                            actions[unit_id] = [np.random.randint(1,4), 0, 0]
                elif unit_id > 1 and unit_id < 1+len(self.relic_fragments):
                    actions[unit_id] = [move_to_position(unit_pos, self.relic_fragments[unit_id-2], self.prev_positions[unit_id]), 0, 0]
        ...
        self.prev_points = obs['team_points'][self.team_id] 

        self.prev_actions = actions.copy()
        self.prev_positions = unit_positions.copy()
